...
class CosAdapter extends AbstractAdapter
{
    ...
    public function __construct($config)
    {
        ...
        if(isset($config['acl'])) $this->acl = $config['acl'];
        if(isset($config['internal'])) $this->internal = (bool) $config['internal'];
        if(isset($config['ssl'])) $this->ssl = (bool) $config['ssl'];
        if(isset($config['cdn']) && filter_var($config['cdn'], FILTER_VALIDATE_DOMAIN)) $this->cdn = $config['cdn'];
        if(isset($config['cdn_token'])) $this->cdnToken = $config['cdn_token'];
    }
